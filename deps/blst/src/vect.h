...
/* ants-client local patch: __BLST_NO_ASM__ checked first.
 *
 * Upstream order is (x86_64|aarch64) -> _WIN64 -> __BLST_NO_ASM__ ->
 * fallback. That makes the 64-bit hardware branches win on aarch64
 * even when __BLST_NO_ASM__ is set, which yields LIMB_T_BITS=64; but
 * no_asm.h only defines llimb_t (the double-width type used by the
 * portable C multiplication) when LIMB_T_BITS==32, so we get
 * "unknown type name 'llimb_t'". The upstream portable path is
 * written for 32-bit limbs.
 *
 * Hoisting __BLST_NO_ASM__ to the top forces the 32-bit-limb branch
 * on every platform when we ask for the portable build, which is
 * what the ants-client root CMakeLists.txt always asks for. The asm
 * paths on x86_64/aarch64/_WIN64 still work for builds that DON'T
 * define __BLST_NO_ASM__ — but we never make such a build. */
