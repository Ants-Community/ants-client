# BearSSL — vendored subset (ECDSA verify on NIST prime curves + SHA-384/512)
...
generic-prime **ECDSA signature verifier** and the **SHA-384/512 hash**, for
checking the vendor signature chains in TEE attestation quotes (RFC-0005).
Intel TDX signs with ECDSA P-256 (handled by `deps/p256-m`); **AMD SEV-SNP
signs its attestation report with ECDSA P-384 over a SHA-384 digest**, neither
of which the other vendored crypto provides — hence this dependency. (libsodium
exposes SHA-512 but not SHA-384, which is a distinct IV + truncation.)

## What is vendored

The dependency closures of `br_ecdsa_i31_vrfy_raw` (over the generic-prime
curve implementation `br_ec_prime_i31` — the constant-time "i31", 31-bit-limb,
big-integer backend) and of `br_sha384` / `br_sha512`. The sources are
...
| `src/hash/sha2big.c` → `src/sha2big.c` | `br_sha384` + `br_sha512` (SHA-2 512-family) |
...
minimal set above (27 `.c` for the verifier; the SHA-384/512 leg adds 3 more — `sha2big.c` + `dec64be.c` + `enc64be.c`). It links and verifies all 14 curated P-384
