 ...
 */
/*
 * ANTS local patch (diff against upstream p256-m @44af59e): add
 * `!defined(__aarch64__)`. Upstream's `__ARM_ARCH_PROFILE == 'A'` test also
 * matches 64-bit Cortex-A (Apple Silicon, arm64 Linux — including the macOS
 * CI runners), but the asm in this block is 32-bit-ARM Thumb (`.syntax
 * unified`, low-register `"+l"`/`"l"` constraints) and AArch64 toolchains
 * reject it ("invalid output constraint '+l' in asm"). AArch64 has no
 * __ARM_FEATURE_DSP and falls through to the portable C `u32_muladd64`
 * below, which is correct and — for our verify-only use over public
 * attestation data — sufficient. Worth reporting upstream.
 */
