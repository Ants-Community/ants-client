...
test-vector emitter, on the `chain_vectors` pattern — per RFC-0008 §8
the vectors are generated by the **compiled** `ants_reputation`
library, never transcribed by hand. Deterministic by construction
(repeated-byte keys and nonces, no clock, no randomness; Ed25519
signing is RFC 8032 deterministic, so the signatures are stable
vectors); every proof, signature, and verdict is re-verified through
the library's own verifier before printing. Covers RFC-0008 v0.7
§11.9 + §4.1 end to end: receipt bodies + dual signatures, `bag_root`
(incl. the empty bag) + inclusion paths, the (A, T) computation under
the DRAFT defaults, the `A ≥ b` proof, and the compact summary with
per-bucket audit pass and fail verdicts. Cross-checked against
hand-computed closed forms (A = 12537; buckets `[[0,192],[1,12345]]`;
selection `[3,1]`) and an eye-decode of the 161-byte summary encoding
against §11.9. The emitted pack is the seventh in `ants-test-vectors`
(`vectors/receipt-bag/identity.json`, commit `fb812f4`).
...
Landed across the engine commit (`860fcb6`) and a same-day follow-up that
corrected two test-harness assertions (an over-narrow malformed-frame
expectation and an oversized-frame fixture that under-sized its buffer);
the engine itself was unchanged. Tests (real Ed25519 + the real Component
#7 G-Set, not mocked) drive an in-process "network" of gossip instances
through their send callbacks — a faithful epidemic terminating by G-Set
dedup, no QUIC — covering the push codec + reject verdicts, view
management, submit-forwards-and-dedups, an A→B→C line that reaches the far
end without bouncing back to the sender, invalid-proof reject-not-forward,
truncated vs over-cap frame rejection, and arg guards. Full suite 19/19
green; 0 warnings on a full rebuild; clang-format clean.
