...
**Status:** scaffold landed; surfaces 1–2 implemented (BDFL interim primary; claim still open). Full API surface + opaque ctx + protocol constants in `include/ants_inference.h`. Surface 1 (commit-at-send) — per-position leaf hashing, the promote-lone-node Merkle root/prove/verify, canonical-CBOR commit encode/decode, and Ed25519 sign/verify — and surface 2 (anti-grinding challenge derivation) — the three beacon-bound PRF derivations (audit selector, strided position set, verifier assignment) over a counter-mode BLAKE3 keystream — are implemented and tested (KAT + independent cross-checks under ASan/UBSan). The remaining two surfaces still validate arguments and return `ANTS_ERROR_NOT_IMPLEMENTED`, landing in subsequent PRs: betting e-process (init/update + discrepancy scoring) → serving runtime + audit capstone.
**Effort:** 4 EM.
